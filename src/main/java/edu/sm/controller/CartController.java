...
public class CartController {
    ...
    // JSP 파일들이 위치한 폴더 경로를 변수로 관리 (배우신 방식)
    String dir = "cart/";

    /**
     * 고객 ID를 받아 장바구니 목록 페이지를 보여줍니다.
     */
    @RequestMapping("")
    ...
    public String addCart(Cart cart) {
        try {
            // product_id, cust_id, product_qt가 cart 객체에 담겨서 들어옵니다.
            cartService.register(cart);
        } catch (Exception e) {
            e.printStackTrace();
        }
        // 작업 완료 후, 다시 해당 고객의 장바구니 목록 페이지로 이동합니다.
        return "redirect:/cart?id=" + cart.getCustId();
    }

    /**
     * 장바구니의 특정 상품을 삭제합니다.
     * cart_id(기본키)를 기준으로 삭제합니다.
     */
    ...
    /**
     * 장바구니의 특정 상품 수량을 변경합니다.
     */
    @RequestMapping("/update")
    public String updateCart(Cart cart) {
        try {
            // cart_id와 변경할 product_qt가 cart 객체에 담겨서 들어옵니다.
